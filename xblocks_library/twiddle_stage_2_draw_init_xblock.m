...
counter_rst_del = xBlock(struct('source', 'Delay', 'name', 'counter_rst_del'), ...
    struct('latency', 0, 'reg_retiming', 'on'), {sync}, {counter_rst});

% delay counter_rst by mux_latency for output sync
sync_delay = xBlock(struct('source', 'Delay', 'name', 'sync_delay'), ...
    struct('latency', negate_latency, 'reg_retiming', 'on'), {counter_rst}, {sync_out});

% Counter for select signal
counter = xBlock(struct('source', 'Counter', 'name', 'counter'), ...
    struct('n_bits', FFTSize - 1 , 'rst', 'on', 'use_behavioral_HDL', 'on','start_count', 0), {counter_rst}, {count});
...
%     struct('latency', negate_latency-1, 'reg_retiming', 'on'), {b_re}, {b_re_bram_del});
% 
% % delay b_im by bram_delay
% b_im_bram_del_sub = xBlock(struct('source', 'Delay', 'name', 'b_im_bram_del'), ...
%     struct('latency', negate_latency-1, 'reg_retiming', 'on'), {b_im}, {b_im_bram_del});


if negate_dsp48e
    xBlock( struct('source', str2func('negate_mux_init_xblock'), 'name', 'negate_mux'), ...
        {[], 1, input_bit_width, input_bit_width-1}, ...
...
mux0 = xBlock(struct('source', 'Mux', 'name', 'mux0'), ...
    struct('latency', negate_latency), {sel, b_re, b_im}, {bw_re_out});
