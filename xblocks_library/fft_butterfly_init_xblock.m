...
output_latency = get_var('output_latency', 'defaults', defaults, varargin{:});
...
elseif output_latency > 0
    apbw = xSignal();
    ambw = xSignal();
    
    sync_out = xSignal();
    
    apbw_out = xOutport('a+bw');
    ambw_out = xOutport('a-bw');
    of = xOutport('of');
    sync_out_out = xOutport('sync_out');
    
    xBlock(struct('source', 'Delay', 'name', 'apbw_output_del'), ...
        {'latency', output_latency}, {apbw}, {apbw_out});
    xBlock(struct('source', 'Delay', 'name', 'ambw_output_del'), ...
        {'latency', output_latency}, {ambw}, {ambw_out});
    xBlock(struct('source', 'Delay', 'name', 'sync_output_del'), ...
        {'latency', output_latency}, {sync_out}, {sync_out_out});    
else
    error('Invalid output latency: %d', output_latency)
end
